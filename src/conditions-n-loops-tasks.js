...
function isIsoscelesTriangle(a, b, c) {
  ...
  return canFormTriangle && isIsosceles;
}
...
function isContainNumber(num, digit) {
  ...
  for (let index = 0; index < numStr.length; index += 1) {
    if (parseInt(numStr[index], 10) === digit) {
      result = true;
      break;
    }
  }
...
function getSpiralMatrix(size) {
  ...
  while (startColumn <= endColumn && startRow <= endRow) {
    for (let i = startColumn; i <= endColumn; i += 1) {
      results[startRow][i] = numberToFillIn;
      numberToFillIn += 1;
    }
    startRow += 1;
    for (let i = startRow; i <= endRow; i += 1) {
      results[i][endColumn] = numberToFillIn;
      numberToFillIn += 1;
    }
    endColumn -= 1;
    for (let i = endColumn; i >= startColumn; i -= 1) {
      results[endRow][i] = numberToFillIn;
      numberToFillIn += 1;
    }
    endRow -= 1;
    for (let i = endRow; i >= startRow; i -= 1) {
      results[i][startColumn] = numberToFillIn;
      numberToFillIn += 1;
    }
    startColumn += 1;
  }
...
function sortByAsc(arr) {
  ...
  for (let i = 0; i < length; i += 1) {
    ...
    let j = i - 1;
    for (j; j >= 0 && result[j] > currentElement; j -= 1) {
      result[j + 1] = result[j];
    }
...
function shuffleChar(str, iterations) {
  ...
  while (iterationsCount > 0) {
    ...
    result = oddPart + evenPart;
    if (result === str) {
      return shuffleChar(str, iterations % (iterations - iterationsCount));
    }
  }
